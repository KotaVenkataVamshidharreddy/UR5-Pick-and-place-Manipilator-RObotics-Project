...
% Get the Target 
    ...
    [res , TargPos]= vrep.simxGetObjectPosition(id,handles.ur5Cuboids(iCuboid),...
                          relativToRef,vrep.simx_opmode_buffer);
    vrchk(vrep, res,true);
    [res , Targtheta]= vrep.simxGetObjectOrientation(id,handles.ur5Cuboids(iCuboid),...
                            relativToRef,vrep.simx_opmode_buffer);
    vrchk(vrep, res,true);
...
%% Get the arm to be above the Cuboid   
   ...
   %Get Target Joint Value 
	q =handles.ur5Robot.ikunc(G); %1*6 vector
   
   %Move
    moveFrame( id, vrep, G, handles.Target, relativToRef); %move Target Frame
    pause(0.5);
    MoveUR5Joints(id, vrep, handles, q );%Then move the Arm  
